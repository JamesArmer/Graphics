#define _USE_MATH_DEFINES

#include "BluePlayers.h"

BluePlayers::BluePlayers() {

}

void BluePlayers::Display() {
    glDisable(GL_CULL_FACE);

    //Blue side
    glPushMatrix();
    glTranslatef(50.f, 0.f, 3.f);
    glPushMatrix();//Player1
    glRotatef(270.f, 0.f, 1.f, 0.f);
    animatedPlayer(joint1, legX1, true);
    glPopMatrix();//~1
    glPushMatrix();//Player2
    glTranslatef(-7.f, 0.f, 0.f);
    glRotatef(90.f, 0.f, 1.f, 0.f);
    animatedPlayer(joint2, legX2, true);
    glPopMatrix();//~2
    glPushMatrix();//Player3
    glTranslatef(7.f, 0.f, 5.f); 
    glRotatef(180.f, 0.f, 1.f, 0.f);
    person(true);
    glPopMatrix();//~3
    glPushMatrix();//Player4
    glTranslatef(0.f, 0.f, 12.f);
    glRotatef(180.f, 0.f, 1.f, 0.f);
    person(true);
    glPopMatrix();//~4
    glPopMatrix();//all

    glEnable(GL_CULL_FACE);
}

void BluePlayers::Update(const double& deltaTime) {
    aT = fmod(aT + deltaTime, animationTime);          // update time in animation cycle

    float aS = 4.f * aT / animationTime;         // calculate stage (out of x)

    if (aS < 0.1f) {//reset people
        legX1 = 0.5f;
        legX2 = 1.5f;
    }
    else if (aS < 0.5f) {
        joint1 = 90 * aS;//leg going back
        legX1 += -0.04f;
    }
    else if (aS < 1.f) {//leg going forward
        joint1 = -45 * (aS-0.5);
        legX1 += 0.04f;
    }
    else if ((aS > 2.f) && (aS < 2.5f)) {
        joint2 = 90 * (aS-2);//leg going back
        legX2 += -0.04f;
    }
    else if ((aS > 2.5f) && (aS < 3.f)) {//leg going forward
        joint2 = -45 * (aS - 2.5);
        legX2 += 0.04f;
    }
}

void BluePlayers::animatedPlayer(float joint, float legX, bool isBlue) {
    glPushMatrix();
    glScalef(0.08f, 0.08f, 0.08f);
    glTranslatef(0.f, 18.f, 0.f);
    glPushMatrix();//1 - head
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(1.f, 0.9f, 0.9f);//skin colour
    glScalef(1.f, 1.1f, 1.f);
    glutSolidSphere(2.f, 50.f, 50.f);
    glPopMatrix();//~1
    glPushMatrix();//2 - body
    glPushAttrib(GL_CURRENT_BIT);
    if (isBlue) {
        glColor3f(0.f, 0.f, 1.f);//blue
    }
    else {
        glColor3f(1.f, 0.f, 0.f);//red
    }
    glTranslatef(0.f, -6.f, 0.f);
    Box(6.f, 8.f, 2.f);
    glPopAttrib();
    glPushMatrix();//3 - left arm
    glTranslatef(-4.f, -5.f, 0.f);
    cylinder(8.f, 0.7f);
    glPopMatrix();//~3
    glPushMatrix();//4 - right arm
    glTranslatef(4.f, -5.f, 0.f);
    cylinder(8.f, 0.7f);
    glPopMatrix();//~4
    glPopAttrib();
    glPushMatrix();//5 - left leg
    glTranslatef(-1.5f, -7.f, legX);
    glRotatef(joint, 1.f, 0.f, 0.f);//thigh rotation
    cylinder(3.5f, 1.f);//thigh
    glTranslatef(0.f, -0.3f, 0.f);
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(1.f, 0.9f, 0.9f);//skin colour
    glutSolidSphere(0.7f, 50.f, 50.f);//knee
    glTranslatef(0.f, -3.5f, 0.f);
    cylinder(3.f, 0.8f);//shin
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(0.f, 0.f, 0.f);//black
    glTranslatef(0.f, -0.7f, 0.5f);
    Box(1.5f, 1.f, 3.f);//foot
    glPopAttrib();
    glPopMatrix();//~5
    glPushMatrix();//6 - right leg
    glPopAttrib();
    glTranslatef(1.5f, -7.f, 0.f);
    cylinder(3.5f, 1.f);//thigh
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(1.f, 0.9f, 0.9f);//skin colour
    glTranslatef(0.f, -0.3f, 0.f);
    glutSolidSphere(0.7f, 50.f, 50.f);//knee
    glTranslatef(0.f, -3.5f, 0.f);
    cylinder(3.f, 0.8f);//shin
    glTranslatef(0.f, -0.7f, 0.5f);
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(0.f, 0.f, 0.f);//black
    Box(1.5f, 1.f, 3.f);//foot
    glPopAttrib();
    glPopAttrib();
    glPopMatrix();//~6
    glPopMatrix();//~2
    glPopMatrix();//~all
}

void BluePlayers::person(bool isBlue) {
    glPushMatrix();
    glScalef(0.08f, 0.08f, 0.08f);
    glTranslatef(0.f, 18.f, 0.f);
    glPushMatrix();//1 - head
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(1.f, 0.9f, 0.9f);//skin colour
    glScalef(1.f, 1.1f, 1.f);
    glutSolidSphere(2.f, 50.f, 50.f);
    glPopMatrix();//~1
    glPushMatrix();//2 - body
    glPushAttrib(GL_CURRENT_BIT);
    if (isBlue) {
        glColor3f(0.f, 0.f, 1.f);//blue
    }
    else {
        glColor3f(1.f, 0.f, 0.f);//red
    }
    glTranslatef(0.f, -6.f, 0.f);
    Box(6.f, 8.f, 2.f);
    glPopAttrib();
    glPushMatrix();//3 - left arm
    glTranslatef(-4.f, -5.f, 0.f);
    cylinder(8.f, 0.7f);
    glPopMatrix();//~3
    glPushMatrix();//4 - right arm
    glTranslatef(4.f, -5.f, 0.f);
    cylinder(8.f, 0.7f);
    glPopMatrix();//~4
    glPopAttrib();
    glPushMatrix();//5 - left leg
    glTranslatef(-1.5f, -7.f, 0.f);
    cylinder(3.5f, 1.f);//thigh
    glTranslatef(0.f, -0.3f, 0.f);
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(1.f, 0.9f, 0.9f);//skin colour
    glutSolidSphere(0.7f, 50.f, 50.f);//knee
    glTranslatef(0.f, -3.5f, 0.f);
    cylinder(3.f, 0.8f);//shin
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(0.f, 0.f, 0.f);//black
    glTranslatef(0.f, -0.7f, 0.5f);
    Box(1.5f, 1.f, 3.f);//foot
    glPopAttrib();
    glPopMatrix();//~5
    glPushMatrix();//6 - right leg
    glPopAttrib();
    glTranslatef(1.5f, -7.f, 0.f);
    cylinder(3.5f, 1.f);//thigh
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(1.f, 0.9f, 0.9f);//skin colour
    glTranslatef(0.f, -0.3f, 0.f);
    glutSolidSphere(0.7f, 50.f, 50.f);//knee
    glTranslatef(0.f, -3.5f, 0.f);
    cylinder(3.f, 0.8f);//shin
    glTranslatef(0.f, -0.7f, 0.5f);
    glPushAttrib(GL_CURRENT_BIT);
    glColor3f(0.f, 0.f, 0.f);//black
    Box(1.5f, 1.f, 3.f);//foot
    glPopAttrib();
    glPopAttrib();
    glPopMatrix();//~6
    glPopMatrix();//~2
    glPopMatrix();//~all
}

void BluePlayers::cylinder(float h, float r) {
    float res = 0.1 * M_PI;           // resolution (in radians: equivalent to 18 degrees)
    float x = r, z = 0.f;           // initialise x and z on right of cylinder centre
    float t = 0.f;                  // initialise angle as 0

    do
    {
        glBegin(GL_QUADS);          // new QUAD
            // Create first points
        glVertex3f(x, h, z);    // top
        glVertex3f(x, 0.f, z);  // bottom
        // Iterate around circle
        t += res;               // add increment to angle
        x = r * cos(t);           // move x and z around circle
        z = r * sin(t);
        // Close quad
        glVertex3f(x, 0.f, z);  // bottom
        glVertex3f(x, h, z);    // top
        glEnd();                    // end shape
    } while (t <= 2 * M_PI);        // for a full circle (360 degrees)

    glBegin(GL_POLYGON);
    while (t <= 2 * M_PI) {
        glVertex3f(x, h, z);    // top
        t += res;               // add increment to angle
        x = r * cos(t);           // move x and z around circle
        z = r * sin(t);
    }        // for a full circle (360 degrees)
    glEnd();
}

void BluePlayers::Box(float sx, float sy, float sz) {
    glPushMatrix();
    glScalef(sx, sy, sz);
    glutSolidCube(1.f);
    glPopMatrix();
}
